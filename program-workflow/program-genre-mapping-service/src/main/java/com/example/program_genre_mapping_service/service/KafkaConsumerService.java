package com.example.program_genre_mapping_service.service;

import com.example.program_genre_mapping_service.model.GenreEntity;
import com.example.program_genre_mapping_service.model.SimplifiedProgram;
import com.example.program_genre_mapping_service.model.TransformedProgram;
import com.example.program_genre_mapping_service.repository.GenreRepository;
...
public class KafkaConsumerService {
    ...
    private Map<String, GenreEntity> genreIdMapping;
    private Map<String, GenreEntity> genreNameMapping;

    @Autowired
    public KafkaConsumerService(KafkaTemplate<String, String> kafkaTemplate, GenreRepository genreRepository, ObjectMapper objectMapper) {
        this.kafkaTemplate = kafkaTemplate;
        this.genreRepository = genreRepository;
        this.objectMapper = objectMapper;
        this.objectMapper.setSerializationInclusion(JsonInclude.Include.NON_NULL);
        this.genreIdMapping = new HashMap<>();
        this.genreNameMapping = new HashMap<>();
        this.objectMapper.enable(SerializationFeature.INDENT_OUTPUT);
        this.objectMapper.setSerializationInclusion(JsonInclude.Include.NON_EMPTY);
        loadGenreMapping();
    }

    @KafkaListener(topics = "TRANSFORMED_PROGRAM_DATA", groupId = "genre-mapping-group", containerFactory = "kafkaListenerContainerFactory")
    public void consume(TransformedProgram transformedProgram, @Header("kafka_receivedMessageKey") String key) {
        ...
        try {
            ...
            kafkaTemplate.send("GENRE_MAPPED_DATA", key, json);
            System.out.println("Message sent to GENRE_MAPPED_DATA topic with key: " + key);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private SimplifiedProgram createSimplifiedProgram(TransformedProgram program) {
        ...
        List<String> genreIds = program.getGenres().stream()
                .map(genre -> {
                    String providerGenreId = genre.getGenreId();
                    String genreValue = genre.getValue().trim().toLowerCase();
                    System.out.println("Processing genre with providerGenreId: " + providerGenreId + " and value: " + genreValue);

                    GenreEntity mappedGenreEntity = genreIdMapping.get(providerGenreId);
                    if (mappedGenreEntity != null) {
                        return mappedGenreEntity.getId();
                    } else {
                        mappedGenreEntity = genreNameMapping.get(genreValue);
                        if (mappedGenreEntity != null) {
                            return mappedGenreEntity.getId();
                        } else {
                            System.out.println("No mapping found for genreId: " + providerGenreId + " or value: " + genreValue);
                            return null;
                        }
                    }
                })
        ...
        simplifiedProgram.setGenres(genreIds);
        ...
        simplifiedProgram.setAdvisories(copyAdvisories(program.getAdvisories()));
    ...
    private List<SimplifiedProgram.Advisory> copyAdvisories(List<TransformedProgram.Advisory> advisories) {
        if (advisories == null) {
            return null;
        }
        return advisories.stream().map(advisory -> {
            SimplifiedProgram.Advisory newAdvisory = new SimplifiedProgram.Advisory();
            newAdvisory.setId(advisory.getId());
            newAdvisory.setValue(advisory.getValue());
            newAdvisory.setRatingsBody(advisory.getRatingsBody());
            newAdvisory.setLang(advisory.getLang());
            return newAdvisory;
        }).collect(Collectors.toList());
    }
    ...
    private void loadGenreMapping() {
        try {
            ...
            for (GenreEntity genre : genreEntities) {
                for (var ref : genre.getExternalRefs()) {
                    if ("gracenote".equals(ref.getSystem()) && "id".equals(ref.getRefName())) {
                        genreIdMapping.put(ref.getId(), genre);
                    }
                }
                for (var name : genre.getNames()) {
                    genreNameMapping.put(name.getValue().toLowerCase(), genre);
                }
            }
        } catch (Exception e) {
            System.out.println("Error loading genre mapping: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
